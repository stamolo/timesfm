...
def run_step_11():
    ...
    - ИСПРАВЛЕНО: Добавлена корректная нормализация и денормализация целевой переменной (Y).
    """
    logger.info("---[ Шаг 11: Поиск аномалий в скользящем окне ]---")
    try:
        ...
        # --- ПАРАМЕТРЫ ВЫБОРА МОДЕЛИ, НОРМАЛИЗАЦИИ И INTERCEPT ---
        ...
        logger.info(f"Модель будет обучаться с intercept (свободным членом): {fit_intercept_option}")
        # ---------------------------------------------
        ...
        for block_num, block_df in enumerate(data_blocks, 1):
            ...
            # --- Фабрика моделей с учетом опции fit_intercept ---
            try:
                if model_type == 'ridge':
                    params = model_params.get('ridge', {})
                    model = Ridge(fit_intercept=fit_intercept_option, **params)
                elif model_type == 'lasso':
                    params = model_params.get('lasso', {})
                    model = Lasso(fit_intercept=fit_intercept_option, **params)
                elif model_type == 'elasticnet':
                    params = model_params.get('elasticnet', {})
                    model = ElasticNet(fit_intercept=fit_intercept_option, **params)
                else:  # 'linear'
                    model = LinearRegression(fit_intercept=fit_intercept_option)
            except Exception as e:
                logger.error(f"Ошибка при создании модели: {e}. Будет использована LinearRegression.")
                model = LinearRegression(fit_intercept=fit_intercept_option)

            is_model_fitted = False
            scaler = None
            y_scaler = None  # Инициализация скейлера для целевой переменной
            ...
            for i in tqdm(range(min_window_size, len(block_df), window_step),
                ...
                if should_retrain:
                    ...
                    if normalization_type == 'min_max':
                        ...
                        # --- ИЗМЕНЕНИЕ: Нормализация целевой переменной ---
                    ...
                    # Обучаем модель на нормализованных X и Y
                ...
                # --- ИЗМЕНЕНИЕ: Денормализация (обратное преобразование) предсказаний ---
                if y_scaler:
                ...
                # --- ИСПРАВЛЕНО: Полностью переработан блок расчета вклада признаков, ---
                # чтобы корректно обрабатывать все комбинации нормализации (y_scaler)
                # и наличия свободного члена (fit_intercept_option).
                if is_linear_model:
                    if y_scaler:
                        # --- НОВОЕ: Проверка типа скейлера для выбора правильной логики ---
                        if isinstance(y_scaler, StandardScaler):
                            # Логика для Z-Score нормализации (StandardScaler)
                            # y_pred = (y_pred_scaled * scale) + mean
                            denormalized_coef = model.coef_ * y_scaler.scale_
                            feature_contributions = X_predict_scaled * denormalized_coef
                            df.loc[analysis_block.index, contribution_cols] = feature_contributions

                            denormalized_intercept = 0.0
                            if fit_intercept_option:
                                denormalized_intercept = model.intercept_ * y_scaler.scale_ + y_scaler.mean_
                            else:
                                denormalized_intercept = y_scaler.mean_

                            denormalized_intercept_scalar = denormalized_intercept[0] if isinstance(denormalized_intercept, np.ndarray) else denormalized_intercept
                            df.loc[analysis_block.index, intercept_col_name] = denormalized_intercept_scalar

                        elif isinstance(y_scaler, MinMaxScaler):
                            # Логика для Min-Max нормализации
                            ...
                            denormalized_intercept = 0.0
                            if fit_intercept_option:
                                denormalized_intercept = model.intercept_ * data_range + y_scaler.data_min_
                            else:
                                denormalized_intercept = y_scaler.data_min_

                            denormalized_intercept_scalar = denormalized_intercept[0] if isinstance(denormalized_intercept, np.ndarray) else denormalized_intercept
                            df.loc[analysis_block.index, intercept_col_name] = denormalized_intercept_scalar

                    else:
                        # Y не был нормализован, расчеты производятся в исходном масштабе.
                        feature_contributions = X_predict_scaled * model.coef_
                        df.loc[analysis_block.index, contribution_cols] = feature_contributions
                        intercept_value = model.intercept_ if fit_intercept_option else 0.0
                        df.loc[analysis_block.index, intercept_col_name] = intercept_value

                # Расчет остатков теперь корректен, т.к. predictions в том же масштабе, что и target_col
